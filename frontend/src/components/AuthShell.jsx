...
const trustSignals = [
  { icon: LockKeyhole, label: 'Private storage', value: 'Controlled access' },
  { icon: Fingerprint, label: 'Integrity checks', value: 'SHA-256 review' },
  { icon: ShieldCheck, label: 'Verified access', value: 'OTP protected' },
];
...
const AuthShell = ({ eyebrow, title, description, children, footer }) => {
  ...
  return (
    <div className="auth-stage security-grid min-h-screen px-4 py-4 sm:px-6 sm:py-6 lg:p-8">
      <button
        onClick={toggle}
        className="fixed right-4 top-4 z-20 inline-flex h-11 w-11 items-center justify-center rounded-lg border border-white/70 bg-white/80 text-slate-500 shadow-sm backdrop-blur transition hover:-translate-y-0.5 hover:text-sky-700 dark:border-white/10 dark:bg-slate-900/80 dark:text-slate-300 dark:hover:text-emerald-300 sm:right-6 sm:top-6"
        title={dark ? 'Switch to light mode' : 'Switch to dark mode'}
      >
        {dark ? <Sun className="h-5 w-5" /> : <Moon className="h-5 w-5" />}
      </button>

      <main className="mx-auto grid min-h-[calc(100vh-2rem)] w-full max-w-7xl overflow-hidden rounded-xl border border-white/70 bg-white/80 shadow-sm backdrop-blur dark:border-white/10 dark:bg-slate-950/70 lg:grid-cols-[1.08fr_0.92fr]">
        <section className="auth-rail relative hidden overflow-hidden px-10 py-11 text-white lg:flex lg:flex-col lg:justify-between">
          ...
          <div className="relative">
            <Link to="/login" className="inline-flex items-center gap-3">
              <span className="grid h-14 w-14 place-items-center rounded-lg border border-white/15 bg-white/10 shadow-md">
                <img src="/logo.png" className="h-10 w-10 object-contain" alt="SecureTransfer logo" />
              </span>
              <span>
                <span className="block text-xs font-semibold uppercase text-cyan-100/80">Secure file workspace</span>
            ...
            <div className="mt-20 max-w-xl animate-[floatUp_0.7s_ease]">
              ...
              <h1 className="text-4xl font-semibold leading-[1.08]">
                A secure workspace for encrypted file sharing.
              </h1>
              <p className="mt-6 max-w-lg text-lg leading-8 text-slate-200">
                Upload, verify, and share sensitive files with clear access controls and integrity checks.
              </p>
            </div>
          </div>
          ...
          <div className="relative grid gap-3 xl:grid-cols-3">
            {trustSignals.map((signal) => (
              <div key={signal.label} className="rounded-lg border border-white/10 bg-white/10 p-4 backdrop-blur">
        ...
        <section className="flex min-h-full items-center justify-center px-4 py-16 sm:px-8 lg:px-12">
          <div className="w-full max-w-md animate-[fadeIn_0.5s_ease]">
            <Link to="/login" className="mb-8 flex items-center gap-3 lg:hidden">
              <span className="grid h-12 w-12 place-items-center rounded-lg border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-slate-900">
                <img src="/logo.png" className="h-9 w-9 object-contain" alt="SecureTransfer logo" />
              </span>
              <span>
                <span className="block text-xs font-semibold uppercase text-cyan-700 dark:text-cyan-300">Secure file workspace</span>
            ...
            <div className="surface-glass rounded-lg border border-white/80 p-5 dark:border-white/10 sm:p-7">
              {children}
            </div>
